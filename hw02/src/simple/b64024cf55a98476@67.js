...
function _3(Plot,data1){return(
Plot.plot({
  y: {grid: true, label: "count"},
  marks: [
    Plot.rectY(data1, Plot.binX({y: "count"}, {x: "Year"})),
...
function _5(Plot,plot1,data1){return(
Plot.plot({  
	...
	y: {grid: true, label: "count"},  
	marks: [    
		Plot.rectY(data1, Plot.binX({y:"count"}, { x:"Year", interval:1, fill:"Gender", tip: true })),    
		Plot.gridY({ interval: 1, stroke: "white", strokeOpacity: 0.5 }),
    Plot.ruleY([0])
	 ]
})
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["Plot","data1"], _3);
  ...
  main.variable(observer()).define(["Plot","plot1","data1"], _5);
  return main;
}
